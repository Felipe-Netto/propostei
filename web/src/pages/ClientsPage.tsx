...
import { Users, Plus, ArrowLeft, Phone, Mail, FileText, ChevronRight } from 'lucide-react'
...
export function ClientsPage() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      const created = await createClient(companyId, input)
            ...
            <ul className="divide-y divide-slate-100">
              {clients.map((client) => (
                <li key={client.id}>
                  <button
                    onClick={() =>
                      navigate(`/empresas/${companyId!}/clientes/${client.id}`, {
                        state: { companyName },
                      })
                    }
